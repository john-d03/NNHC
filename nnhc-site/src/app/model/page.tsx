...
export default function ModelPage() {
  const t = useT();
  const SECTIONS = [
    { id: "vision", label: t("model.rail.vision", "The vision") },
    { id: "principles", label: t("model.rail.principles", "Principles") },
    { id: "star", label: t("model.rail.star", "Star Model") },
    { id: "method", label: t("model.rail.method", "Method") },
  ];
  return (
    <main id="main">
      {/* ═══════════════ HERO ═══════════════ */}
        ...
        <div className="relative mx-auto max-w-[88rem] px-5 md:px-10 pt-16 md:pt-24 pb-16 md:pb-20">
          <div className="flex items-center gap-3 mb-10 md:mb-14">
            <span className="notice-pill">
              <span className="label-num">{t("model.hero.pillIndex", "P1")}</span>
              <span className="sep" aria-hidden />
              <span>{t("model.hero.pillLabel", "Vision and operating structure")}</span>
            </span>
          </div>
          ...
          >
            {t("model.hero.titlePrefix", "A single continuous")}{" "}
            <span className="h-italic" style={{ color: "var(--color-electric)" }}>
              {t("model.hero.titleItalic", "fabric of care.")}
            </span>
          </h1>
      ...
      <ScrollSpyRail sections={SECTIONS}>
        ...
        >
          <SectionTitle
            kicker={t("model.rail.vision", "The vision")}
            title={t("model.vision.sectionTitle", "One fabric, not five domains.")}
          />
          <div className="mt-12 grid md:grid-cols-12 gap-10 md:gap-16">
            <Reveal className="md:col-span-7">
              <h3 id="vision-heading" className="sr-only">
                {t("model.rail.vision", "The vision")}
              </h3>
            ...
            <Reveal className="md:col-span-5" delay={120}>
              <aside className="card p-7 md:p-8 relative overflow-hidden">
                ...
                />
                <span className="label relative">{t("model.vision.asideLabel", "Note")}</span>
                <h4 className="h-display text-2xl mt-3 mb-4 leading-tight relative">
                  {t("model.vision.asideTitlePrefix", "What this is, and what it")}{" "}
                  <span className="h-display-italic text-electric">
                    {t("model.vision.asideTitleItalic", "is not.")}
                  </span>
                </h4>
                <p className="text-sm text-ink-soft leading-relaxed relative">
                  {t(
                    "model.vision.asideBody",
                    "Kerala's palliative care movement, including IMA Arike, has demonstrated that communities can hold responsibility for end-of-life care. NNHC builds on that proof but answers a different question - where palliative care meets a person at the end of life, NNHC works upstream.",
                  )}
                </p>
              </aside>
            </Reveal>
        ...
        >
          <div className="mx-auto max-w-[88rem] px-5 md:px-10 py-20 md:py-28">
            <SectionTitle
              kicker={t("model.rail.principles", "Principles")}
              title={t("model.principles.sectionTitle", "Five guiding principles.")}
            />
            <h3 id="principles-heading" className="sr-only">
              {t("model.rail.principles", "Principles")}
            </h3>
        ...
        >
          <SectionTitle
            kicker={t("model.rail.star", "Star Model")}
            title={t("model.star.sectionTitle", "Five stakeholder groups, one shared centre.")}
          />
          <h3 id="star-heading" className="sr-only">
            {t("model.rail.star", "Star Model")}
          </h3>
        ...
        >
          <div className="mx-auto max-w-[88rem] px-5 md:px-10 py-20 md:py-28">
            <SectionTitle
              kicker={t("model.rail.method", "Method")}
              title={t("model.method.sectionTitle", "From vision to action - needs, services, gaps.")}
            />
            <h3 id="method-heading" className="sr-only">
              {t("model.rail.method", "Method")}
            </h3>
                ...
                <Reveal as="li" key={s.n} delay={i * 100}>
                  <article className="card p-7 h-full flex flex-col group">
                    ...
                    <h4 className="h-display text-2xl mb-3 leading-tight text-balance">
                      {t(`threeSteps.${i + 1}.title`, s.title)}
                    </h4>
                    <p className="text-sm text-ink-soft leading-relaxed">
                      {t(`threeSteps.${i + 1}.body`, s.body)}
                    </p>
                  </article>
                </Reveal>
              ))}
            ...
            <div className="mt-16 card p-8 md:p-12 grid md:grid-cols-12 gap-8 items-center relative overflow-hidden">
              ...
              <div className="md:col-span-7 relative">
                <span className="label text-electric">{t("model.method.ctaKicker", "What comes next")}</span>
                <h3
                  className="h-display mt-3 text-balance"
                  style={{ fontSize: "clamp(1.5rem, 3.5vw, 2.5rem)", lineHeight: 1.1 }}
                >
                  {t("model.method.ctaTitle", "The model compiles into five operating tracks.")}
                </h3>
                <p className="mt-4 lede max-w-[48ch]">
                  {t(
                    "model.method.ctaBody",
              ...
              <div className="md:col-span-5 relative md:text-right">
                <Link href="/mission" className="btn btn-primary">
                  {t("model.method.ctaButton", "Mission specs")} <span aria-hidden>→</span>
                </Link>
              </div>
            </div>
          </div>
